...
- Implementar patrones **Creacionales, Estructurales y Comportamentales**
...
| **🎭 Comportamentales** | **Cómo** interactúan objetos | Comunicación entre objetos |
...
## 🎭 Patrones Comportamentales - Lista
...
## 🛠️ Práctica: Implementando Patrones GoF + PoSA

### Objetivo
Implementar **patrones GoF** (Factory, Strategy) y **patrones PoSA** (Functional Decomposition + Reverse Proxy Routing) en un sistema de e-commerce.
...
## 🎯 Escenario: MiTienda Online Avanzada

### Arquitectura del sistema:

1. **🛒 Servicio de Productos** (GoF: Factory + Strategy)
2. **👤 Servicio de Usuarios** (GoF: Singleton + Observer)
3. **💳 Servicio de Pagos** (GoF: Strategy + Factory)
4. **🚪 Gateway (Nginx)** (PoSA: Reverse Proxy Routing)

**Patrones aplicados:**
- **GoF:** Factory Method, Strategy, Singleton, Observer
- **PoSA:** Functional Decomposition, Reverse Proxy Routing

---

## 📝 Paso 1: Estructura del Proyecto Avanzada
...
## 🏗️ Paso 2: Servicio de Productos (Factory + Strategy)

**Archivo:** `productos/Models/Product.cs`
```csharp
...
{
    public override string GetCategory() => "Books";
}
```

**Archivo:** `productos/Factories/ProductFactory.cs`
```csharp
// Factory Method Pattern
public abstract class ProductFactory
{
    public abstract Product CreateProduct(int id, string name, decimal price);
}

public class ElectronicsFactory : ProductFactory
{
    public override Product CreateProduct(int id, string name, decimal price)
    {
        return new Electronics { Id = id, Name = name, Price = price };
    }
}
...
public class ClothingFactory : ProductFactory
{
    public override Product CreateProduct(int id, string name, decimal price)
    {
        return new Clothing { Id = id, Name = name, Price = price };
    }
}
```
...
## 🎭 Paso 2: Strategy Pattern para Descuentos

**Archivo:** `productos/Strategies/DiscountStrategy.cs`
```csharp
// Strategy Pattern
...
public class NoDiscountStrategy : IDiscountStrategy
{
    ...
    public string GetDiscountType() => "Sin descuento";
}
...
public class PercentageDiscountStrategy : IDiscountStrategy
{
    ...
    public string GetDiscountType() => $"{_percentage}% descuento";
}
...
public class FixedAmountDiscountStrategy : IDiscountStrategy
{
    ...
    public string GetDiscountType() => $"${_amount} descuento";
}
```

---

## 🎯 Paso 3: API Controller con Patrones

**Archivo:** `productos/Program.cs`
```csharp
...
// Endpoint con Factory Method
app.MapGet("/productos/{category}", (string category) =>
{
    if (!factories.ContainsKey(category.ToLower()))
        return Results.BadRequest("Categoría no válida");

    var factory = factories[category.ToLower()];
    var products = new List<Product>
    {
        factory.CreateProduct(1, $"Producto {category} 1", 100m),
        factory.CreateProduct(2, $"Producto {category} 2", 200m)
    };

    return Results.Ok(new {
        category,
        products,
        pattern = "Factory Method aplicado"
    });
});

// Endpoint con Strategy Pattern
app.MapPost("/productos/{id}/aplicar-descuento",
    ([FromRoute] int id, [FromBody] DiscountRequest request) =>
{
    IDiscountStrategy strategy = request.Type switch
    {
    ...
    decimal originalPrice = 150m; // Simular precio del producto
    decimal discountedPrice = strategy.ApplyDiscount(originalPrice);
    ...
    return Results.Ok(new {
        ...
        pattern = "Strategy Pattern aplicado"
    });
});

public record DiscountRequest(string Type, decimal Value);
...
## � Paso 4: Verificación de Patrones

### Probar Factory Method:
```bash
curl http://localhost:8001/productos/electronics
curl http://localhost:8001/productos/clothing
```

### Probar Strategy Pattern:
```bash
# Descuento porcentual (10%)
curl -X POST http://localhost:8001/productos/1/aplicar-descuento \
...
### **Patrones GoF:**
1. **¿Cómo beneficia Factory Method?**
   - Desacopla creación de productos de lógica de negocio
   - Permite agregar nuevas categorías sin modificar código existente

2. **¿Por qué Strategy es útil para descuentos?**
   - Permite cambiar algoritmos de descuento dinámicamente
   - Facilita agregar nuevos tipos de descuento

### **Patrones PoSA:**
3. **¿Qué ventajas da Functional Decomposition?**
   - Servicios independientes, escalables por separado
   - Equipos pueden trabajar en paralelo

4. **¿Por qué usar Reverse Proxy?**
   - Punto de entrada unificado
   - Balanceado de carga y enrutamiento flexible

---

## 🏆 Desafío Extra: Combinar Patrones

### Implementar Observer en el servicio de usuarios:
...
public class EmailNotificationObserver : IUserObserver
{
    public void OnUserRegistered(User user)
    {
        Console.WriteLine($"📧 Enviando email de bienvenida a {user.Email}");
    }
}

public class UserService
{
    private List<IUserObserver> _observers = new();
...
## 📊 Entregables Avanzados

1. **Screenshots** de todos los endpoints funcionando
2. **Diagrama** mostrando qué patrones se usan dónde
3. **Código fuente** completo con comentarios explicando patrones
4. **Reflexión escrita** (500 palabras) sobre beneficios observados
5. **(Bonus)** Implementación del desafío Observer
